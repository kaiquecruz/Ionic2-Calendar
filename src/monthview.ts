...
export class MonthViewComponent implements ICalendarComponent, OnInit, OnChanges {
    ...
    getHighlightClass(date:IMonthViewRow):string {
        ...
        if (date.hasEvent) {
            for (let i = 0; i < date.events.length; i++) {
                let dia:any = date.events[i];

                if(dia.entregue){
                    className = 'monthview-entregue-with-event';
                }
                else if(dia.pendente){
                    className = 'monthview-pendente-with-event';
                }
                else if(dia.cancelado){
                    className = 'monthview-cancelado-with-event';
                }   
            }
        }
